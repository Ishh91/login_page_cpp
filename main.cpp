...
class LoginManager
{
 public:
    ...
    void login()
    {
        ...
        int usrID = checkFile(userNameAttempt, "user.txt");
        if(usrID != 0)
        {
            cout << "Password:";
            cin >> passwordAttempt;

            int pwdID = checkFile(passwordAttempt, "pswd.txt");
            if(usrID == pwdID)
            {
...
{
    LoginManager app;
    app.saveFile("user@email.com","user.txt",1);
    app.saveFile("Password","pswd.txt",1);
    app.saveFile("ishant@email.com","user.txt",2);
    app.saveFile("ishant","pswd.txt",2);
